...
2. Вынести в настройку обрабатывать ли формулы, сейчас обрабатываются.
3. Добавить в настройки возможность выбора листа Excel-документа.
4. Сделать отдельный класс для динамического массива `IntDinamicArray` для повторного использования.
5. Добавить поддержку чтения файлов по URL (HTTP/HTTPS).
6. Добавить ExceptionHandler.
7. Полное покрытие тестам, сейчас минимум для себя.
8. `Dockerfile` с инструкцией запуска с монтированием тома.
9. Можно ещё пофантазировать на тему, а что если читать не столбцы, а строки, или может быть не только Excel-файлы.